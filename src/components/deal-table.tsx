...
import { Loader2, Circle, Trash2, ChevronLeft, ChevronRight, SlidersHorizontal } from "lucide-react";
...
type DealSortCol = "price" | "endoValue" | "endoPerPlat" | "reRolls" | "riven" | "listed" | "updated";
...
const PAGE_SIZE = 100;
const RANK_VALUE_PROFIT_THRESHOLD_KEY = "rank_value_profit_threshold";
...
function dedup(deals: RivenSnapshot[]): RivenSnapshot[] {
...
export function DealTable() {
  ...
  const [sort, setSort] = useState<SortState<DealSortCol>>({ col: "endoPerPlat", dir: "desc" });
  const [profitThreshold, setProfitThreshold] = useState<number>(() =>
    getStoredProfitThreshold(RANK_VALUE_PROFIT_THRESHOLD_KEY)
  );
  ...
  useEffect(() => {
    try {
      localStorage.setItem(RANK_VALUE_PROFIT_THRESHOLD_KEY, String(profitThreshold));
    } catch {
      /* ignore */
    }
  }, [profitThreshold]);
  ...
  const fetchDeals = useCallback(async () => {
    try {
      const statusParam = tab === "all" ? "" : tab === "1plat" ? "&status=1plat" : `&status=${tab}`;
      const priceParam = tab === "1plat" ? "&max_price=1" : "&min_price=2";
      const offset = page * PAGE_SIZE;
      const res = await fetch(`/api/deals?limit=${PAGE_SIZE}&offset=${offset}&liquid=true${statusParam}${priceParam}`);
      const json = await res.json();
      setData(json);
    } catch {
      /* ignore */
  ...
  const handlePurge = useCallback(async () => {
    if (
      !confirm("Remove all liquid deals? The next scan will repopulate. This cannot be undone.")
    )
      return;
    setPurging(true);
    try {
      const res = await fetch("/api/deals/purge", { method: "DELETE" });
      const json = await res.json();
      if (res.ok) {
  ...
  const dealAccessor = useCallback(
    (d: RivenSnapshot, col: DealSortCol): number | string | Date | null => {
      switch (col) {
  ...
  const deals = useSorted(filtered, sort, dealAccessor);
    ...
    <Tabs value={tab} onValueChange={setTab} className="h-full flex flex-col min-h-0">
      ...
      <div className="flex items-center justify-between gap-2 mb-2 shrink-0">
        <TabsList className="h-8">
          ...
          <TabsTrigger value="1plat" className="text-xs h-7 px-3 text-muted-foreground">
            1 plat {data ? `(${data.onePlatCount ?? 0})` : ""}
          </TabsTrigger>
        </TabsList>
      ...
      <div className="rounded-md border border-border/30 overflow-auto flex-1 min-h-0">
        {loading ? (
          ...
          <div className="text-center py-10 text-sm text-muted-foreground">
            No liquid deals found. Run a scan to populate.
          </div>
        ) : (
          <Table>
            <TableHeader className="sticky top-0 bg-card z-10">
              <TableRow className="border-border/20 hover:bg-transparent">
                ...
                <SortableHead column="updated" sort={sort} onSort={(c) => setSort(toggleSort(sort, c))} className="w-16">
                  UPDATED
                </SortableHead>
                <TableHead className="text-xs font-mono h-8 px-2">SELLER</TableHead>
                    ...
                    <div className="flex items-center gap-1">
                      <Circle
                        className={`size-1.5 shrink-0 fill-current ${
                          deal.ownerStatus === "ingame" || deal.ownerStatus === "online"
                            ? "text-neon-green"

...
const WFM_RIVEN_WEAPONS = "https://api.warframe.market/v2/riven/weapons";

async function main() {
  console.log("[WFM] Fetching riven weapons from", WFM_RIVEN_WEAPONS);
  const res = await fetch(WFM_RIVEN_WEAPONS, {
  ...
  console.log(
    "[WFM] Received",
    items.length,
    "riven-eligible weapons (maximum guns for Rank Value & Reroll Value)."
  );
  if (items.length > 0) {
    const byType = {};
    for (const w of items) {
      const t = w.rivenType ?? "unknown";
      byType[t] = (byType[t] || 0) + 1;
    }
    console.log("[WFM] By rivenType:", byType);
  }
  console.log(
    "[WFM] To populate the app, run POST /api/reference/sync (e.g. from Admin page)."
  );
}
